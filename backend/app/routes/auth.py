...
async def youtube_callback(code: str, db: Session = Depends(get_db)):
    ...
    user = crud.get_user_by_email(db, email) or crud.create_user(
        db, username=name, email=email, hashed_password=""
    )

    # Save tokens in DB using CRUD
    crud.create_or_update_social_account(
        db, user_id=user.id, platform="youtube",
        account_id=email,
        access_token=access_token,
    ...
    jwt = create_access_token({"user_id": user.id, "platform": "youtube"})

    return {"message": "YouTube connected", "access_token": jwt, "user": user.id}
